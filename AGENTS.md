...
- **Host binding trap (SPEC R7):** llama-server `--host [IP_ADDRESS]` binds IPv6 link-local only on this box → `[IP_ADDRESS]` is NOT a valid .NET Uri host, curl/Invoke-WebRequest fail. Spawn new eval servers with `--host Scamper` (ASCII name, resolves fine); existing :8080/:8081/:8082 already bound, don't touch. Probe tool: `uv run python scripts/embed_vram_probe.py` (candidate VRAM deltas → `data/embed_vram.json`, SPEC T86/T87). **Probe bug (2026-08-08):** `BASE_URL = http://[{HOST}]:8084` bracket-wraps hostname → IPv6-literal syntax, "Scamper" not IPv6 → unresolvable → every candidate "start timeout". Server itself boots fine via `--host Scamper`. Working recipe: `--host Scamper` then hit `http://[fe80::f0f0:631:e268:23df%8]:8084/health` (zone-id `%8` required). Fix = resolve hostname → IPv6 addr before URL build