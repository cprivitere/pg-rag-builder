"""Wiki page context expansion (Step 5 of metadata-bm25-eval).

General queries retrieve only the top chunks of a wiki page; the answer often
sits in a sibling chunk. `_prepare_general` pulls sibling chunks via
`parent_id` — bounded (2 pages, ~16k chars), id-deduped, non-wiki untouched.
"""
from unittest.mock import patch

from pgrag.rag.pipeline import _expand_wiki_context, _prepare_general
    ...
    return [
        {
            ...
            "type": "wiki",
            "text": f"text {i} " + ("x" * 200),
            "metadata": {"source": "wiki", "parent_id": "wiki_Mushroom Farming"},
        }
        for i in ids
    ]


def test_expansion_pulls_siblings():
    retrieved = _docs(["wiki_Mushroom Farming_Mechanics_chunk_0"])
    store = {
        d["id"]: d for d in (
            _docs(["wiki_Mushroom Farming_Mechanics_chunk_0",
                   "wiki_Mushroom Farming_Mechanics_chunk_1",
                   "wiki_Mushroom Farming_Mechanics_chunk_3"])
        )
    }
    ids, texts, metas, dists = _expand_wiki_context(
        [retrieved[0]["id"]], [retrieved[0]["text"]],
        [retrieved[0]["metadata"]], [0.5], store,
    )
    assert len(ids) == 3
...
def test_expansion_dedupes_retrieved_ids():
    retrieved = _docs(["wiki_Page_A", "wiki_Page_B"])
    store = {
        d["id"]: d for d in (
            _docs(["wiki_Page_A", "wiki_Page_B", "wiki_Page_C"])
        )
    }
    ids, texts, metas, dists = _expand_wiki_context(
...
def test_expansion_bounded_to_two_pages():
    retrieved = [
    ...
    store = {d["id"]: d for d in retrieved + siblings}

    ids, _, _, _ = _expand_wiki_context(
        [r["id"] for r in retrieved],
        [r["text"] for r in retrieved],
        [r["metadata"] for r in retrieved],
        [0.0] * 4,
...
def test_expansion_respects_char_cap():
    big = {"id": "wiki_Big_chunk", "type": "wiki", "text": "z" * 9000,
           "metadata": {"parent_id": "wiki_Big"}}
    store = {"wiki_Big_chunk": big}
    ids, texts, _, _ = _expand_wiki_context(
        ["wiki_Other"], ["other"], [{"parent_id": "wiki_Other"}], [0.0], store,
    )
    # 9000 > remaining budget after other overhead -> dropped
    assert ids == ["wiki_Other"]
    assert sum(len(t) for t in texts) < 16000


def test_expansion_non_wiki_untouched():
    ids, texts, metas, dists = _expand_wiki_context(
...
@patch("pgrag.rag.pipeline._load_parent_index", return_value=({}, {}))
@patch("pgrag.rag.pipeline._expand_wiki_context")
...
def test_prepare_general_expands_wiki(
    mock_retrieve, mock_synth, mock_expand, mock_parent
):
...
def test_expansion_skips_when_no_wiki_parent():
    ids, texts, metas, dists = _expand_wiki_context(
        ["skill_Alchemy"], ["alchemy text"],
        [{"source": "cdn", "table": "skills"}], [0.2],
        {"skill_Alchemy": {"id": "skill_Alchemy", "text": "alchemy"}},
    )
